export function render_main_links(){
    let main_links_blok = document.querySelector('.main_links')
...
export function render_posts(posts){
    let posts_blok = document.querySelector('.tape .posts')

    posts.forEach(post=>{
        let post_blok = document.createElement('div')
        ...
        post.text.forEach(el=>{
            if (post.text.indexOf(el)==0) {
                text_blok.innerHTML += `
        ...
        reply_btn.addEventListener('click', ()=>{
            if (replies_blok.classList.contains('display_none')) {
                replies_blok.classList.remove('display_none')
            } else {
                replies_blok.classList.add('display_none')
            }
        })
...
export function render_pop_posts(){
    let pop_posts = [
        {
            name:'Denim',
    ...
    pop_posts.forEach(post=>{
        pop_posts_blok.innerHTML+=`
        <div class='pop_post'>
...
export function render_tags(){
    let tags_blok  = document.querySelector('.tags .content')
